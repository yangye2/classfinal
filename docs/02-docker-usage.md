...
docker run --rm \
  ...
  ghcr.io/yangye2/classfinal/classfinal:latest encrypt
```
...
docker run --rm \
  ...
  -e PASSWORD=mypassword \
  ghcr.io/yangye2/classfinal/classfinal:latest agent
```

#### 普通运行模式

```bash
docker run --rm ghcr.io/yangye2/classfinal/classfinal:latest run --help
```
...
```bash
# 进入容器 shell
docker run --rm -it ghcr.io/yangye2/classfinal/classfinal:latest /bin/sh

# 执行自定义命令
docker run --rm ghcr.io/yangye2/classfinal/classfinal:latest ls -la /app
```
